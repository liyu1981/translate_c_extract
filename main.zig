...
const usage =
    \\usage:
    \\    tools/translate_c_extract/run.sh <path-to>/<source_header>.h
    \\
...
pub fn main() !u8 {
    ...
    if (debug) {
        ...
        const stderr_file = try std.fs.openFileAbsolute("/dev/null", .{ .mode = .write_only });
        stderr_color_config = std.io.tty.detectConfig(stderr_file);
        stderr = stderr_file.writer();
    }
    ...
    const zig_autogen_content = brk: {
        const result = try zcmd.run(.{
            .allocator = ja,
            .commands = &[_][]const []const u8{
                &.{ "zig", "translate-c", maybe_header_file_path.valueOf() },
            },
        });
    ...
    const autogen_chunks = try parseAutogen(ja, zig_autogen_content);
...
fn parseAutogen(allocator: std.mem.Allocator, content: []const u8) anyerror![]JString {
    var all_content = try JString.newFromSlice(allocator, content);
